function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
    ...
    for (const position of plainTextPositions) {
        ...
        const startPosTag = highlightedContent.indexOf(plainText.substring(startPos, endPos));

        if (startPosTag !== -1) {
            const startTagPos = startPosTag + startPos;
            const endTagPos = startTagPos + (endPos - startPos);

            const startTag = `<mark style="color: white; background-color: yellow;> `;
            const endTag = '</mark>';

            highlightedContent = highlightedContent.substring(0, startTagPos) +
                startTag +
                highlightedContent.substring(startTagPos, endTagPos) +
                endTag +
                highlightedContent.substring(endTagPos);
        }
    }
...
const highlightedResult = highlightHTMLContent(htmlContent, plainText, plainTextPositions);
